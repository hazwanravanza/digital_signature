...
def sign_generic_file():
    ...
    signature_hex = menandatangani(data.decode('latin1'), key_path)

    temp_sig = tempfile.NamedTemporaryFile(delete=False, suffix=".sig", mode='w')
    with open(temp_sig.name, 'w') as f:
...
def verify_generic_file():
    ...
    signature_hex = signature_file.read().decode('utf-8')
